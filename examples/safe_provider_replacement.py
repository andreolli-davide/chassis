...
async def main() -> None:
    ...
    print(
        f"[start] leases={generation_one.lease_count} "
        f"instances={len(harness.plugin_registry.instances())}"
    )
    ...
    print(
        f"[publish] new generation {result.generation_id}, previous {generation_one.generation_id}"
    )
    ...
    print(
        f"[safety] provider-a alive={not provider_a.disposed}, "
        f"generation_refs={instance_a.generation_refs}"
    )
    ...
    print(
        f"[dispose] provider-a disposed={provider_a.disposed} "
        f"provider-b disposed={provider_b.disposed}"
    )
