...
| G26 | A preview computes the complete plan — parsing, migration, validation, catalog resolution, and resolution — with zero mutation of authoritative state, and its decisions match the subsequent apply | `chassis/planning.py`, `tests/planning/test_preview_purity.py::test_preview_leaves_every_authoritative_state_object_unchanged`, `tests/planning/test_preview_parity.py::test_preview_of_a_configuration_matches_the_apply` |
| G27 | Operational telemetry follows one stable signal contract — declared names, required correlation attributes, bounded cardinality — and no secret material can enter any signal | `chassis/telemetry/signals.py`, `tests/telemetry/test_signal_contract.py::test_every_emitted_signal_conforms_to_the_contract`, `tests/telemetry/test_signal_contract.py::test_secrets_never_reach_a_signal` |
