...
from chassis.secrets import REDACTED, StaticSecretProvider
...
def vault():  # type: ignore[no-untyped-def]
    ...
    provider = StaticSecretProvider({"db-password": SECRET}, name="vault")

    @plugin(name="vault", version="1.0.0", provides={"secrets": "1.0.0"})
    async def provide(ctx: PluginContext) -> None:
        ctx.capabilities.provide(SECRETS, provider)
