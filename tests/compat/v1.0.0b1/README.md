# Candidate 1.0.0b1 format fixtures

These documents are the first 1.0 beta candidates for every persisted format
family Chassis documents. They are generated from the current `1.0.0b1`
implementation; unlike the historical `v0.8.1` fixtures, they are review
targets rather than a permanent compatibility baseline.

Regenerate them only through the checked-in generator:

```bash
uv run python tests/compat/v1.0.0b1/generate.py
```
