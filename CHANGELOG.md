...
- **Composition-provided secret providers no longer bypass the shared
  ...
  and cannot appear in telemetry, replay records, or error strings. Providers
  already using a different redactor retain it and also join the harness-owned
  boundary.
...
- **Replay keys reject non-string mapping keys** (R015/R021). The replay key
  ...
  silently collapse `{1: ..., "1": ...}` into one value and one key. Set
  elements retain their scalar types during canonical ordering, so `{1}` and
  `{"1"}` cannot collide either.
...
- **Declarative config is deeply frozen and de-aliased** (R009/R011).
  ...
  configuration. Publication also rechecks an author-constructed `FrozenDict`
  instead of assuming its nested values are immutable.
...
- **The guarantee-to-test gate requires real collected test nodes** (R023).
  ...
  collects; the design table now names one per enforcement reference. Migration
  guide links use the anchors generated by MkDocs.
- **Release delivery is coordinated across PyPI, GitHub, and the documentation
  site.** A verified tag publishes the distributions through trusted publishing,
  creates a GitHub Release carrying the wheel, sdist, and SBOM, then deploys the
  documentation from that same tag. The docs workflow remains independently
  usable for previews and updates from `main`.
